...
import LGICTable from "./LGICTable";
import LGICTable2 from "./LGICTable2";
...
export default function LGICDetails({ id, action, apiName, actions }) {
  ...
  const [detailsLifting, setDetailsLifting] = useState("");
    ...
    const fetchData = async () => {
      ...
      if (transId !== 0 && apiName) {
        ...
        if (response?.Status === "Success") {
          ...
          setBody(myObject?.Body?.filter((item) => item.iType === 1));
          setBody2(myObject?.Body?.filter((item) => item.iType === 2));
          ...
          setDetailsLifting(myObject?.Header[0]?.sDetailsOfLG);
          setSlNo(myObject?.Header[0]?.sSlNoOfLG);
  ...
  const handleSave = async (e) => {
    ...
    const updatedChildData = [...childData1, ...childData2].map(
      (obj, index) => {
        return {
          SlNo: obj.iSLNo,
          Items: obj.iItems,
    ...
    try {
      Swal.fire({
        ...
        if (result.value) {
          handleOpen();
          const response = await postLGIC(formData);
          handleClose();
          if (response?.Status === "Success") {
            Swal.fire({
              title: "Saved",
  ...
  const handleClear = async () => {
    ...
    const response2 = await getFormData({ FormId: 5, iType: 1 });
    if (response2?.Status === "Success") {
      const myObject = JSON.parse(response2?.ResultData);
      setBody(myObject);
    }
    const response3 = await getFormData({ FormId: 5, iType: 2 });
  ...
  const handlePages = async (iType, iTransId) => {
    handleOpen()
    const response = await getPrev_NextDocNo({ iTransId, iType, iForm: 5,iUser });
    if (response?.Status === "Success") {
      const myObject = JSON.parse(response?.ResultData);
  ...
  const handlePrint = async (id) => {
    handleOpen();
    const response = await crystalPrint({ iTransId: id, iFormtype: 5 });
    handleClose();
    if (response?.Status === "Success") {
                  ...
                  <MDBCol lg="3" md="4" sm="6" xs="12">
                    <div className="mb-3">
                      <MDBInput
                        ...
                        autoComplete="off"
                        maxLength={300}
                        label="Detail of Lifting Gear *"
                        value={detailsLifting}
                        onChange={(e) => setDetailsLifting(e.target.value)}
                        labelStyle={{
                          fontSize: "15px",
                        }}
                      />
