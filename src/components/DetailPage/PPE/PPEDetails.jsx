...
export default function PPEDetails({ id, action, apiName, actions }) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      Swal.fire({
        ...
        if (result.value) {
          handleOpen();
          const response = await postPPE(formData);
          handleClose();
          if (response?.Status === "Success") {
            Swal.fire({
              title: "Saved",
  ...
  const handleClear = async () => {
    ...
    const response2 = await getFormData({ FormId: 7, iType: 0 });
  ...
  const handlePages = async (iType, iTransId) => {
    handleOpen();
    const response = await getPrev_NextDocNo({ iTransId, iType, iForm: 7, iUser });
    handleClose();
    if (response?.Status === "Success") {
      const myObject = JSON.parse(response?.ResultData);
  ...
  const handlePrint = async (id) => {
    handleOpen();
    const response = await crystalPrint({ iTransId: id, iFormtype: 7 });
    handleClose();
    if (response?.Status === "Success") {
  ...
  return (
    <>
      ...
      {renderButton(11, "Print", ()=>handlePrint(transId), <PrintIcon />, transId === 0)}
        ...
        <LGICTable data={body} handleChildData={handleChildData} name="" />
