...
import LGICTable from "../LGIC/LGICTable";
import LGICTable2 from "../LGIC/LGICTable2";
...
export default function QHSEDetails({ id, action, apiName, actions }) {
    ...
    const fetchData = async () => {
      ...
      if (transId !== 0 && apiName) {
        ...
        if (response?.Status === "Success") {
          ...
          setActivity(myObject?.Header[0]?.sActivity);
  ...
  const handleSave = async (e) => {
    ...
    try {
      Swal.fire({
        ...
        if (result.value) {
          handleOpen();
          const response = await postQHSE(formData);
          handleClose();
          if (response?.Status === "Success") {
            Swal.fire({
              title: "Saved",
  ...
  const handleClear = async () => {
    ...
    const response2 = await getFormData({ FormId: 6, iType: 1 });
    if (response2?.Status === "Success") {
      const myObject = JSON.parse(response2?.ResultData);
      setBody(myObject);
    }
    const response3 = await getFormData({ FormId: 6, iType: 2 });
  ...
  const handlePages = async (iType, iTransId) => {
    handleOpen();
    const response = await getPrev_NextDocNo({ iTransId, iType, iForm: 6, iUser });
    if (response?.Status === "Success") {
      const myObject = JSON.parse(response?.ResultData);
  ...
  const handlePrint = async (id) => {
    handleOpen();
    const response = await crystalPrint({ iTransId: id, iFormtype: 6 });
    handleClose();
    if (response?.Status === "Success") {
    ...
    <>
      ...
      {renderButton(11, "Print", ()=>handlePrint(transId), <PrintIcon />, transId === 0)}
        ...
        <LGICTable
          ...
          name="Quality"
        />
        <LGICTable2 data={body2} handleChildData={handleChildData} name="HSE" />
