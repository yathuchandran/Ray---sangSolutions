...
import { deletePayGroup, payGroupSummary } from "../../../api/projectApi";
        ...
        {rows.map((header, index) => {
          if (header !== "iId" && header !== "totalRows") {
            ...
            return (
              ...
              >
                {header === "fMin_hr_HalfDay"? "MinHrHalfDay" :header?.slice(1)}
              </TableCell>
            );
          }
        })}
...
export default function PayGroupSummary() {
  ...
  const fetchData = async () => {
   setloading(true)
    setNavigate(false);
    let response = await payGroupSummary({
  ...
  const handleDelete = async () => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        const response = await deletePayGroup(
          {
            iId: ids,
            iUser,
          },
