...
import SpecialOffTimeModal from "./SpecialOffTimeModal";
...
export default function SpecialOffTime() {
  ...
  const fetchData = async () => {
    setloading(true);
    setNavigate(false);
    let response = await DailyBreaksummary({
      DisplayLength: rowsPerPage,
      DisplayStart: displayStart,
      Search: searchQuery,
    });

    if (response?.ResultData && !response.ResultData == "") {
  ...
  const handleDelete = async () => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        const response = await DeleteDailyBreak({
          iIds: ids,
          iUser,
        });
