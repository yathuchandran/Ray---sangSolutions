...
function EnhancedTableToolbar(props) {
  const { name, values, changes, year, setYear } = props;
...
export default function PublicHolidaySummary() {
  ...
  const fetchData = async () => {
    setloading(true);
    setNavigate(false);
    let response = await publicHolidaySummary({
  ...
  }, [details, rowsPerPage, displayStart, searchQuery, year]);
  ...
  const handleDelete = async () => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        const response = await deleteP_Holiday(
          {
            iId: ids,
            iUser,
          },
          ...
          <>
            ...
            >
              <EnhancedTableToolbar
                ...
                numSelected={selected.length} // Provide the numSelected prop3
                year={year}
                setYear={setYear}
              />
