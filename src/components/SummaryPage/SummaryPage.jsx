...
import { deleteSummary, getSummary } from "../../api/apiCall";
import Header from "../Header/Header";
import { buttonColors, secondaryColorTheme } from "../../config";
import IncidentDetailPage from "../DetailPage/Incident/IncidentDetailPage";
import StopCardDetails from "../DetailPage/StopCard/StopCardDetails";
import CloseHSEDetail from "../DetailPage/CloseHSE/CloseHSEDetail";
import LGICDetails from "../DetailPage/LGIC/LGICDetails";
import QHSEDetails from "../DetailPage/QHSE/QHSEDetails";
import PPEDetails from "../DetailPage/PPE/PPEDetails";
import MCCDetails from "../DetailPage/MCC/MCCDetails";
import TBTDetails from "../DetailPage/TBT/TBTDetails";
...
export default function SummaryPage() {
  ...
  const fetchData = async () => {
    ...
    setSelected([]);
    let response;
    if (details?.iScreenId === 7) {
      response = await getSummary({ UserId: iUser }, details?.sName);
    } else {
      response = await getSummary({ UserId: 0 }, details?.sName);
    }
                      ...
                      <TableBody>
                        {visibleRows.map((row, index) => {
                          ...
                          const handleRowDoubleClick = async (event, iId) => {
                            const hasEdit = actions.some(
                              (action) => action.iActionId === 3
                            );
                            if (hasEdit) {
                              setSelected([iId]);
                              setNavigate(true);
