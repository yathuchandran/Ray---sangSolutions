...
const columnOrder = [
  ...
  { id: 'sTime', label: 'Time' },
  { id: 'iType', label: 'Mode' },
  { id: 'sDeviceId', label: 'DeviceID' },
  { id: 'sPhoto', label: 'Image' },
  { id: 'sLocation', label: 'GeoLoc' },
                  ...
                  >
                   ...
                   {columnOrder.map((column) => (
                      <TableCell
                        ...
                        key={column.id}
                        align={column.id === "sPhoto" || column.id === "sLocation" ? "center" : "left"}
                      >
                        {column.id === "sPhoto" ? (
     row[column.id] ? (
        <img
      ...
      onClick={() => handleClickOpen(row[column.id])}
    /> // Display icon if there's a value for sPhoto
      ) : (
        <><BrokenImageIcon sx={{color:secondaryColorTheme}} /><span>No image</span></>// Display text if sPhoto is null or empty
      )
    ) : column.id === "sLocation" ? (
        row[column.id] ? (
            <a href={`https://www.google.com/maps/search/?api=1&query=${row[column.id].replace('_', ',')}`} target="_blank" rel="noopener noreferrer">
      
    
          <LocationOnIcon sx={{ color: secondaryColorTheme }} />
          </a>
        ) : (
          "No location" // Display 'No location' if sLocation is empty
        )
      ) : (
      // Otherwise, display the value or an empty string if null
      row[column.id] || row[column.id] === null ? row[column.id] : ""
    )}
                      </TableCell>
                      ))}
                      </TableRow>
