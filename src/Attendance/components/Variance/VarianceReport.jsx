...
function VarianceReport() {
  ...
  const [exceliProject, setexceliProject] = useState(0)
  ...
  const columnOrder = [
    ...
    { id: 'EmployeeCode', label: 'EmployeeCode' },
    { id: 'ActualProject', label: 'ActualProject' },
    { id: 'ChangedProject', label: 'ChangedProject' },
    { id: 'ActualDate', label: 'ActualDate' },
    { id: 'ChangedDate', label: 'ChangedDate' },
    { id: 'ActualTime', label: 'ActualTime' },
    { id: 'ChangedTime', label: 'ChangedTime' },
    { id: 'ActualType', label: 'ActualType' },
    { id: 'ChangedType', label: 'ChangedType' },
    
  ];
  const excelHeaders = [
    'Employee', 'EmployeeCode', 'ActualProject', 'ChangedProject', 'ActualDate','ChangedDate', 
    'ActualTime', 'ChangedTime', 'ActualType', 'ChangedType'
  ];
  ...
  if (fromDate > today || toDate > today) {
    ...
    try {
      ...
      setexceliProject(iProject)

      const response = await VarienceReport({
  ...
  const handleExcelExport = async () => {
    try {
      ...
      if(formData.FromDate != excelFromDate || formData.ToDate != excelToDate || formData.iEmployee != exceliEmployee || formData.iProject != exceliProject){
        const FromDate = formData.FromDate;
        const ToDate = formData.ToDate;
        const iEmployee = formData.iEmployee;
        const iProject = formData.iProject;
