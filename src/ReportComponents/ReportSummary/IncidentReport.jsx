...
import { CrystalPrint, GetEmployee, GetProject, GetUser, IncidentReactReport } from '../../api/api';
...
function IncidentReport({ data }) {
  ...
  const handleExportToExcel = async () => {
    try {
      const res = await IncidentReactReport({
        DisplayLength: 0,
        DisplayStart: 0,
        FromDate: FromDate,
        ToDate: TODate,
                        ...
                        <TextField
                          ...
                          InputProps={{
                            ...
                            style: {
                              ...
                              paddingBottom: "12px", // Adjust padding bottom to center vertically
                        ...
                        {filteredRows && filteredRows.length > 0 ? (
                          <TableRow>
                            {Object.keys(filteredRows[0])
                              .filter((key) => key !== "totalRows" && key !== "sPath" && key !== "Action" && key !== "Action_Des" && key !== "Des_Damage" && key !== "Des_incident" && key !== "Details_Persons" && key !== "ImmediateCauses" && key !== "Others" && key !== "Potential_Rate" && key !== "Severity_Rate" && key !== "UnderlyingCauses") // Exclude sPath from the keys
                              .map((key) => (
                                <TableCell
                                  key={key}
                                  align={"left"}
                        ...
                        {filteredRows && filteredRows.length > 0 && filteredRows
                          .map((row, index) => {
                            return (
                              ...
                              sx={{ cursor: "pointer",
                            marginBottom:"10px" }}
                              key={row.iId} // Fi Use row.id instead of row.iId
                              >
                                ...
                                {Object.keys(row)
                                  ...
                                  .map((key, idx) => (
                                    ...
                                    sx={{
                                      ...
                                      row[key] === "Low" ? "#32A848" : // Light green
                                      row[key] === "Moderate" ? "#3269A8" : // Light blue
                                      row[key] === "High" ? "#FF0000" : // Light red
                                      "inherit" :
                                      "white",
                                  color: key === "RiskLevel" && (row[key] === "Low" || row[key] === "Moderate" || row[key] === "High") ? "white" : "inherit",
                                    }}
                                      style={{
