...
import { CrystalPrint, GetAllEmployee, GetProject, LGICReport } from '../../api/api';
...
function LiftingGear({ data, name }) {
                          ...
                          {filteredRows && filteredRows.length > 0 ? (
                            <TableRow>
                              {Object.keys(filteredRows[0])
                                .filter((key) => key !== "totalRows" && key !== "sPath" && key !== "DetailsOfLG" && key !== "SlNoOfLG") // Exclude sPath from the keys
                                .map((key) => (
                                  <TableCell
                                  key={key}
                                  align="left"
                          ...
                          {filteredRows && filteredRows.length > 0 && filteredRows
                            .map((row, index) => {
                              return (
                                ...
                                sx={{ cursor: "pointer",
                              marginBottom:"10px" }}
                                key={row.iId} //  row.id instead of row.iId
                                >
