...
import { CrystalPrint, GetProject, GetUser, StopcardReactReport } from '../../api/api';
...
function StopCardReport({ data }) {
  ...
  const handleExportToExcel = async () => {
    try {
      const res = await StopcardReactReport({
        DisplayLength: 0,
        DisplayStart: 0,
        FromDate: FromDate,
        ToDate: TODate,
  ...
  const suggestionRistLevel = [
    { iId: 3, sName: "show All" },
    { iId: 1, sName: "Stopcard" },
    { iId: 2, sName: "NearMiss" },
  ];
                        ...
                        {filteredRows && filteredRows.length > 0 ? (
                          <TableRow>
                            {Object.keys(filteredRows[0])
                                    .filter((key) => key !== "totalRows" && key !== "sPath" && key!=="iTransId") // Exclude sPath from the keys
                                    .map((key) => (
                                <TableCell
                                  key={key}
                                  align={"left"}
