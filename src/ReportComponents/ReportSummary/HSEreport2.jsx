...
import { CrystalPrint, GetAllEmployee, GetEmployee, GetProject, GetUser, GetcloseStatus, HSEReactReport } from '../../api/api';
...
function HSEreport({ data }) {
  ...
  // const [ex_FromDate, setEx_FromDate] = useState(0)
  // const [ex_ToDate, setEx_ToDate] = useState(0)
  // const [ex_Project, setEx_Project] = useState(0)
  // const [ex_initiatedBy, setEx_initiatedBy] = useState(0)
  // const [ex_iActionBy, setEx_iActionBy] = useState(0)
  // const [ex_iRisklevel, setEx_iRisklevel] = useState(0)
  // const [ex_istatus, setEx_istatus] = useState(0)
                          ...
                          <TextField
                            ...
                            InputProps={{
                              ...
                              style: {
                                ...
                                paddingTop: "0px", // Adjust padding top to center vertically
                                paddingBottom: "12px",
                    ...
                    <MDBCol lg="3" md="4" sm="6" xs="12">
                      <div className="mb-3">
                        ...
                        <AutocloseStatus
                          ...
                          value={closeStatus}
                          onChangeName={handleStatusChange}
                        />
                      </div>
                    </MDBCol>
              ...
              <Box sx={{ width: "95%", margin: "auto", marginTop: "30px" }}>
                ...
                <Paper sx={{ width: "100%", mb: 2 }}>
                    ...
                    >
                      <TableHead>
                        {ProjectsList && ProjectsList.length > 0 ? (
                          <TableRow>
                            {Object.keys(ProjectsList[0])
                              .filter(
                                (key) =>
                                  !["totalRows", "sPath", "Observation", "ActionReq", "ProjectDes"].includes(
                                    key
                                  )
                              )
                              .map((key) => (
                      ...
                      <TableBody>
                        {filteredRows && filteredRows.length > 0 && filteredRows.map((row, index) => {
                          return (
                            <TableRow
                              hover
                              role="checkbox"
                            ...
                            >
                              {Object.keys(row)
                                .filter((key) => key !== "totalRows" && key !== "sPath" && key !== "Observation" && key !== "ActionReq" && key !== "ProjectDes") // Exclude unwanted columns
                                .map((key, idx) => (
                                  <TableCell
                                    sx={{
                                      padding: "0px",
                                      ...
                                      backgroundColor: key === "RiskLevel" ?
                                        (row[key] === "Low" ? "#32A848" : row[key] === "Medium" ? "#3269A8" : row[key] === "High" ? "#FF0000" : "inherit") :

                                        key === "Status" ?
                                          (row[key] === "Open" ? "#32A848" : row[key] === "Closed" ? "#FF0000" : "inherit") :
                                          "white",
                                      color: (key === "RiskLevel" && (row[key] === "Low" || row[key] === "Medium" || row[key] === "High")) ||
                                        (key === "Status" && (row[key] === "Open" || row[key] === "Closed")) ?
                                        "white" :
                                        "inherit"
                                    }}
                                    style={{
                                  ...
                                  >
                                    {key === "sImages" ? (
                                      ...
                                      <Button sx={{ padding: "0px" }} onClick={() => handleClosedpdf(row[key])}>
                                        <PictureAsPdfIcon sx={{ padding: "0px" }} />
                                      </Button>
                                    ) : (
                                      row[key]
                  ...
                  <TablePagination
                    ...
                    component="div"
                    count={ProjectsList && ProjectsList.length > 0 ? ProjectsList[0].totalRows : 0}
