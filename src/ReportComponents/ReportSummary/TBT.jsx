...
import { CrystalPrint, GetAllEmployee, GetProject, GetSupervisor, MCCReactReport, TBTReactReport } from '../../api/api';
...
import Supervisor from '../AutoComplete/Suprvisor';
import Swal from "sweetalert2";
import { getActionBasedOnuser } from '../../api/settingsApi';



function TBT({ data, name }) {
  ...
  const handleExportToExcel = async () => {
    try {
      const res = await TBTReactReport({
        DisplayLength: 0,
        DisplayStart: 0,
        FromDate: FromDate,
        ToDate: TODate,
  ...
  const fetchData = async (initialData) => {
    ...
    try {
      const res = await TBTReactReport({
        ...
        iSupervisor: initialData.iSupervisor,
        Search: ""
      });
      setProjectLists(res);
      handleClose()
                          ...
                          {filteredRows && filteredRows.length > 0 ? (
                            <TableRow>
                              {Object.keys(filteredRows[0])
                                .filter((key) => key !== "totalRows" && key !== "sPath") // Exclude sPath from the keys
                                .map((key) => (
                                  <TableCell
                                  key={key}
                                  align="left"
